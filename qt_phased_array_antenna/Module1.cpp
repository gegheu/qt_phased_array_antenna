...
void Module1::initSerial()
{
    m_serialPort = new SerialPort("Module1_Serial", this); // 设置唯一实例ID

    connect(m_serialPort, &ICommunication::dataReceived,this, &Module1::handleSerialDataReceived);
    connect(m_serialPort, &ICommunication::connectStatus,this, &Module1::handleOpenSerialResult);
}

void Module1::initTcp()
{
    m_tcp = new Tcp("Module1_TCP", this); // 设置唯一实例ID

    connect(m_tcp, &ICommunication::dataReceived, this, &Module1::handleTcpDataReceived);
    connect(m_tcp, &ICommunication::connectStatus, this, &Module1::handleTcpConnectedResult);
}

// 串口配置按钮点击
void Module1::on_serialConfigButton_clicked()
{
...
// 串口连接结果处理
void Module1::handleOpenSerialResult(const QString& instanceId, bool result, const QString& errStr)
{
    if (result) {
...
// 串口发送按钮点击
void Module1::on_serialSendButton_clicked()
{
...
// 串口数据接收处理
void Module1::handleSerialDataReceived(const QString& instanceId, const QByteArray& data)
{
    // 使用 instanceId 来区分是哪个实例的数据
    qDebug() << "Data received from:" << instanceId;
    ...
    if (!data.isEmpty()) {
        ...
        ui->serialRxDisplay->setText(QStringLiteral("rx:") + QString::number(m_serialRxNum));
        appendSerialLog(result, false);
    }
}

// 串口日志添加
void Module1::appendSerialLog(const QString& str, bool isSend)
{
...
// 串口清空日志按钮点击
void Module1::on_serialClearLogButton_clicked()
{
    ui->serialLogDisplay->clear();
}

// 串口清空发送数据按钮点击
void Module1::on_serialClearTxDataButton_clicked()
{
    ui->serialSendDataDisplay->clear();
}

// 串口清空计数按钮点击
void Module1::on_serialClearCountButton_clicked()
{
...
// TCP配置按钮点击
void Module1::on_tcpConfigButton_clicked()
{
...
// TCP连接按钮点击
void Module1::on_tcpConnectButton_clicked()
{
...
// TCP断开连接按钮点击
void Module1::on_tcpDisconnectButton_clicked()
{
    Tcp* tcp = qobject_cast<Tcp*>(m_tcp);
    if (!tcp) return;
...
// TCP发送按钮点击
void Module1::on_tcpSendButton_clicked()
{
    if (m_tcp->isConnected()) {
...
// TCP数据接收处理
void Module1::handleTcpDataReceived(const QString& instanceId, const QByteArray& data)
{
    if (m_tcpHexDisplay) {
        // 显示十六进制格式
...
// TCP连接结果处理
void Module1::handleTcpConnectedResult(const QString& instanceId, bool result, const QString& errorInfo)
{
    if (result) {
...
// TCP清空日志按钮点击
void Module1::on_tcpClearLogButton_clicked()
{
    ui->tcpLogDisplay->clear();
}

// TCP清空发送数据按钮点击
void Module1::on_tcpClearTxDataButton_clicked()
{
    ui->tcpSendDataEdit->clear();
}

// 获取串口参数列表
QVariantList Module1::getSerialParaList()
{
...
// 获取TCP参数列表
QVariantList Module1::getTcpParaList()
{
    QVariantList paraList;
...
// 协议测试函数
void Module1::test(VFProtocol::VFModuleFrame data)
{
