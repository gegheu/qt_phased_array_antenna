...
qint64 CommunicationManager::sendFrame(const QString& id, const QByteArray& frame, QString* errStr) {
    if (errStr) errStr->clear();

    if (frame.isEmpty()) {
        if (errStr) *errStr = QStringLiteral("frame is empty");
        return -1;
    }

    ICommunication* device = getDevice(id);
    if (!device) {
        if (errStr) *errStr = QStringLiteral("device is not registered: %1").arg(id);
        return -1;
    }

    if (!device->isConnected()) {
        if (errStr) *errStr = QStringLiteral("device is not connected: %1").arg(id);
        return -1;
    }

    qint64 written = device->write(frame);
    if (written < 0 && errStr) {
        *errStr = QStringLiteral("send failed: %1").arg(id);
    }
    return written;
}
