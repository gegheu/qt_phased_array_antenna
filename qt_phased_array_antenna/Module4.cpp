...
void Module4::initSerial()
{
    m_serialPort = new SerialPort("Module4_Serial", this); // 设置唯一实例ID

    connect(m_serialPort, &ICommunication::dataReceived, this, &Module4::handleSerialDataReceived);
    connect(m_serialPort, &ICommunication::connectStatus, this, &Module4::handleOpenSerialResult);
}

void Module4::initTcp()
{
    m_tcp = new Tcp("Module4_TCP", this); // 设置唯一实例ID

    connect(m_tcp, &ICommunication::dataReceived, this, &Module4::handleTcpDataReceived);
    connect(m_tcp, &ICommunication::connectStatus, this, &Module4::handleTcpConnectedResult);
}
// 串口配置按钮点击
void Module4::on_serialConfigButton_clicked()
{
...
// 串口开关按钮点击
void Module4::on_serialSwitch_clicked()
{
...
// 串口连接结果处理
void Module4::handleOpenSerialResult(const QString& instanceId, bool result, const QString& errStr)
{
    if (result) {
...
// 串口发送按钮点击
void Module4::on_serialSendButton_clicked()
{
...
// 串口数据接收处理
void Module4::handleSerialDataReceived(const QString& instanceId, const QByteArray& data)
{
    if (!data.isEmpty()) {
        QString result;
        if (m_serialHexDisplay) {
...
// 串口日志添加
void Module4::appendSerialLog(const QString& str, bool isSend)
{
...
// 串口清空日志按钮点击
void Module4::on_serialClearLogButton_clicked()
{
    ui->serialLogDisplay->clear();
}

// 串口清空发送数据按钮点击
void Module4::on_serialClearTxDataButton_clicked()
{
    ui->serialSendDataDisplay->clear();
}

// 串口清空计数按钮点击
void Module4::on_serialClearCountButton_clicked()
{
...
// TCP配置按钮点击
void Module4::on_tcpConfigButton_clicked()
{
...
// TCP连接按钮点击
void Module4::on_tcpConnectButton_clicked()
{
...
// TCP断开连接按钮点击
void Module4::on_tcpDisconnectButton_clicked()
{
    Tcp* tcp = qobject_cast<Tcp*>(m_tcp);
    if (!tcp) return;
...
// TCP发送按钮点击
void Module4::on_tcpSendButton_clicked()
{
    if (m_tcp->isConnected()) {
...
// TCP数据接收处理
void Module4::handleTcpDataReceived(const QString& instanceId, const QByteArray& data)
{
    if (m_tcpHexDisplay) {
        // 显示十六进制格式
...
// TCP连接结果处理
void Module4::handleTcpConnectedResult(const QString& instanceId, bool result, const QString& errorInfo)
{
    if (result) {
...
// TCP清空日志按钮点击
void Module4::on_tcpClearLogButton_clicked()
{
    ui->tcpLogDisplay->clear();
}

// TCP清空发送数据按钮点击
void Module4::on_tcpClearTxDataButton_clicked()
{
    ui->tcpSendDataEdit->clear();
}

// 获取串口参数列表
QVariantList Module4::getSerialParaList()
{
...
// 获取TCP参数列表
QVariantList Module4::getTcpParaList()
{
    QVariantList paraList;
...
// 协议测试函数
void Module4::test(VFProtocol::VFModuleFrame data)
{
