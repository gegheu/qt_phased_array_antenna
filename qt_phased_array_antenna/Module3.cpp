...
void Module3::initSerial()
{
    m_serialPort = new SerialPort("Module3_Serial", this); // 设置唯一实例ID

    connect(m_serialPort, &ICommunication::dataReceived, this, &Module3::handleSerialDataReceived);
    connect(m_serialPort, &ICommunication::connectStatus, this, &Module3::handleOpenSerialResult);
}

void Module3::initTcp()
{
    m_tcp = new Tcp("Module3_TCP", this); // 设置唯一实例ID

    connect(m_tcp, &ICommunication::dataReceived, this, &Module3::handleTcpDataReceived);
    connect(m_tcp, &ICommunication::connectStatus, this, &Module3::handleTcpConnectedResult);
}

// 串口配置按钮点击
void Module3::on_serialConfigButton_clicked()
{
...
// 串口发送按钮点击
void Module3::on_serialSendButton_clicked()
{
...
// 串口数据接收处理
void Module3::handleSerialDataReceived(const QString& instanceId, const QByteArray& data)
{
    if (!data.isEmpty()) {
        QString result;
        if (m_serialHexDisplay) {
...
// 串口日志添加
void Module3::appendSerialLog(const QString& str, bool isSend)
{
...
// 串口清空日志按钮点击
void Module3::on_serialClearLogButton_clicked()
{
    ui->serialLogDisplay->clear();
}

// 串口清空发送数据按钮点击
void Module3::on_serialClearTxDataButton_clicked()
{
    ui->serialSendDataDisplay->clear();
}

// 串口清空计数按钮点击
void Module3::on_serialClearCountButton_clicked()
{
...
// TCP配置按钮点击
void Module3::on_tcpConfigButton_clicked()
{
...
// TCP连接按钮点击
void Module3::on_tcpConnectButton_clicked()
{
...
// TCP断开连接按钮点击
void Module3::on_tcpDisconnectButton_clicked()
{
    Tcp* tcp = qobject_cast<Tcp*>(m_tcp);
    if (!tcp) return;
...
// TCP发送按钮点击
void Module3::on_tcpSendButton_clicked()
{
    if (m_tcp->isConnected()) {
...
// TCP数据接收处理
void Module3::handleTcpDataReceived(const QString& instanceId, const QByteArray& data)
{
    if (m_tcpHexDisplay) {
        // 显示十六进制格式
...
// TCP连接结果处理
void Module3::handleTcpConnectedResult(const QString& instanceId, bool result, const QString& errorInfo)
{
    if (result) {
...
// TCP清空日志按钮点击
void Module3::on_tcpClearLogButton_clicked()
{
    ui->tcpLogDisplay->clear();
}

// TCP清空发送数据按钮点击
void Module3::on_tcpClearTxDataButton_clicked()
{
    ui->tcpSendDataEdit->clear();
}

// 获取串口参数列表
QVariantList Module3::getSerialParaList()
{
...
// 获取TCP参数列表
QVariantList Module3::getTcpParaList()
{
    QVariantList paraList;
...
// 协议测试函数
void Module3::test(VFProtocol::VFModuleFrame data)
{
